...
const ButtonDownload: React.FC<Props> = ({ imageUrl, imageSize }) => {
  ...
  return (
    <button
      ...
      className="cursor-pointer group/download absolute bottom-3 right-3 flex gap-1 px-4 py-3 bg-gradient-to-r from-indigo-700 via-purple-700 to-pink-700 text-[#f1f1f1] rounded-3xl hover:bg-opacity-70 font-semibold shadow-lg shadow-cyan-200 active:shadow-inner transition-all duration-300"
    >
      <ImageDownIcon className="h-7 w-7 text-white" />
      <div className="absolute text-xs uppercase scale-0 rounded-2xl py-2 px-2 bg-[#0c0e9b]  mb-3 w-20 text-center font-bold bottom-full right-2 group-hover/download:scale-100 origin-bottom transition-all duration-300 shadow-lg before:content-[''] before:absolute before:top-full before:left-2/4 before:w-3 before:h-3 before:border-solid before:bg-[#0c0e9b] before:rotate-45 before:-translate-y-2/4 before:-translate-x-2/4">
        {readableSize}
      </div>
    </button>
  );
